# Handoff: NexaMe Marketing Website

## Overview
This package is a developer handoff for the **NexaMe** marketing website homepage —
*NexaMe, The Human Operating System*, a luxury AI-driven self-discovery product.
...
- `nexame-website-preview.html` — the full homepage prototype (open in any browser).
...
- **Background:** translucent near-black with `backdrop-filter: blur(14px)`.
  At scrollY≤12 → `rgba(11,11,12,0.4)`, no border. After scroll →
  `rgba(11,11,12,0.86)` + `1px` bottom border `rgba(176,141,62,0.18)`. Transitions
  over 240ms.
- **Logo:** 34×34 gold app-icon (`nexame-icon-gold.png`, `border-radius:8px`) +
  wordmark "NEXAME" in the display serif, `24px`, `letter-spacing:0.14em`.
...
- **Left column** (animates in with `nx-fade-up 0.9s`):
  - Eyebrow: "Your Human Operating System" — mono, `12px`, `letter-spacing:0.22em`,
    uppercase, gold.
  ...
  - Paragraph: body `18px`, `line-height:1.6`, `--text-secondary`, `max-width:46ch`,
    margin `24px 0 32px`. Copy: "NexaMe helps you create a living manual that
...
- **Right column:** `nexame-symbol.png` (gold monolith), `max-width:440px`,
  `filter: drop-shadow(0 0 60px rgba(176,141,62,0.35))`, animation `nx-float 6s`
  infinite (translateY 0↔-14px).
...
#### 3. ValueGrid
- **Background:** `--surface-1` (#111113) with `1px` gold-subtle top+bottom borders,
  padding `72px 0`.
...
- **Card:** `--surface-2` (#1A1A1D), `1px` border `--border-default`, radius `16px`,
...
- **Background:** `--warm-ivory` (#F5F1E8), black text, padding `80px 0`.
...
- **Left:** H2 (near-black) "Your Manual. Your Story. Organized. Beautiful."
  ...
  each = gold icon (`#8A6B2A`) + bold `15px` near-black title + `13px` `#5a564d`
...
- **Right:** a **dark Manual preview card** on the ivory bg — `--near-black` bg,
  ...
  Sanchez" (display serif `30px`) + caption "Human Operating System". Then 4 rows
...
- Bottom row: "© 2024 NexaMe. All rights reserved." · "The Human Operating System"
...
- **Colors:** Near Black `#0B0B0C`, Antique Gold `#B08D3E`, Warm Ivory `#F5F1E8`,
  Cosmic Green `#1A5C4E`. Surfaces `#111113 / #1A1A1D / #232327`. Text `#F5F1E8`
  (primary), `#B7B4AC` (secondary), `#6A6A9F` (tertiary). Gold ramp
  `#EAD9A8 / #D4B36A / #B08D3E / #8A6B2A / #5C4718`.
- **Gradients:** gold `linear-gradient(135deg,#EAD9A8,#B08D3E 45%,#8A6B2A)`; gold
  radial glow; green `linear-gradient(135deg,#2F7A68,#1A5C4E)`; ink.
- **Borders:** subtle `rgba(176,141,62,0.18)`, default `rgba(245,241,232,0.10)`,
  strong `rgba(176,141,62,0.45)`.
- **Type:** display = Canela (→ Cormorant Garamond), body = Söhne (→ Hanken
  Grotesk), mono = Söhne Mono (→ IBM Plex Mono). Scale H1 72 / H2 40 / H3 24 /
...
- **Shadows/glow:** `--shadow-lg: 0 18px 48px rgba(0,0,0,0.55)`;
  `--glow-gold: 0 0 40px rgba(176,141,62,0.28)`; `--glow-gold-sm: 0 0 16px ...0.25`.
- **Motion:** ease-standard `cubic-bezier(0.4,0,0.2,1)`, ease-out
  `cubic-bezier(0.16,1,0.3,1)`; durations 140 / 240 / 420ms.

## Assets
In `assets/` (originals from `github.com/aracelissm/nexame`):
- `logos/nexame-logo.png` — primary gold monolith + wordmark (dark bg only)
- `logos/nexame-symbol.png` — monolith symbol (hero art)
- `logos/nexame-icon-gold.png` — square app icon (nav/footer)
- `imagery/homepage-hero.png`, `imagery/manual-hero.png` — cinematic hero art
> The NexaMe logos are rendered gold-on-black; use only on dark surfaces. No flat
> vector logo was provided.

## ⚠️ Substitutions (pending real brand assets)
- **Fonts** — Canela / Söhne / Söhne Mono are commercial and were **not** provided.
  Closest Google Fonts substitute: Cormorant Garamond / Hanken Grotesk / IBM Plex
  Mono. Swap for licensed files (`next/font/local`) when available.
- **Icons** — NexaMe's bespoke line-icon set exists only as raster art. The
...
```
app/
  layout.js      # next/font (Cormorant Garamond, Hanken Grotesk, IBM Plex Mono)
                 # exposes --font-cormorant / --font-hanken / --font-plex-mono on <html>;
                 # imports globals.css; sets metadata + themeColor #0B0B0C
...
- Icon component wraps `lucide-react`; keep a concept→glyph map so swapping to real
  NexaMe icons later is a one-file change.
...
- `nexame-website-preview.html` — the homepage prototype (source of truth).
